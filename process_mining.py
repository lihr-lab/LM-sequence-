...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--cluster-member-limit", type=int, default=30, help="Other complete sessions kept per cluster. 0 means all.")
...
def event_status(event: dict[str, Any]) -> int:
    return parse_int(event.get("status_code"))
...
def status_summary(
    ...
    by_status = Counter(str(status) for status in known_statuses)
    by_family = Counter(status_family(status) for status in known_statuses)
    api_status_counter: Counter[tuple[str, str]] = Counter()
    for api, status in zip(api_sequence, statuses):
        if status > 0:
            api_status_counter[(api, str(status))] += 1
    return {
        "known_status_count": len(known_statuses),
        "unknown_status_count": max(0, len(raw_sequence) - len(known_statuses)),
        "status_distribution": dict(sorted(by_status.items())),
        "status_family_distribution": dict(sorted(by_family.items())),
        "top_api_statuses": [
...
def sequence_events(session: dict[str, Any]) -> list[dict[str, Any]]:
    events = []
        ...
        events.append(
            {
                "step": index + 1,
                "api": api,
                "status_code": status_sequence[index] if index < len(status_sequence) else 0,
                "raw_request": raw_sequence[index] if index < len(raw_sequence) else "",
            }
        )
    return events


def merge_status_summaries(sessions: list[dict[str, Any]], top_n: int = 20) -> dict[str, Any]:
    ...
    for session in sessions:
        ...
        for api, status in zip(api_sequence, statuses):
            if status > 0:
                ...
                family_counter[status_family(status)] += 1
                api_status_counter[(api, str(status))] += 1
            else:
                unknown += 1
    return {
        "known_status_count": known,
...
def session_from_json(session_obj: dict[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(sequence_obj, dict) and sequence_obj:
        for _, event in sorted(sequence_obj.items(), key=lambda item: int(item[0])):
            if isinstance(event, dict):
                raw = str(event.get("full_api_request") or event.get("token") or event.get("api_request_pattern") or "").strip()
                if raw:
                    raw_sequence.append(raw)
                    status_sequence.append(event_status(event))
    if not raw_sequence:
        candidate_sequence = session_obj.get("full_api_sequence", [])
        if not isinstance(candidate_sequence, list) or not candidate_sequence:
            candidate_sequence = session_obj.get("api_sequence", [])
        raw_sequence = candidate_sequence if isinstance(candidate_sequence, list) else []
        status_sequence = [0 for _ in raw_sequence]
    raw = [str(item).strip() for item in raw_sequence if str(item).strip()]
    apis = [token_to_api(item) for item in raw if token_to_api(item)]
    if len(status_sequence) != len(raw):
        status_sequence = [0 for _ in raw]
    return {
        ...
        output.append(
            {
                ...
                "events": sequence_events(sessions[index]),
                "status_summary": status_summary(
                    sessions[index].get("raw_sequence", []),
                    sessions[index].get("api_sequence", []),
                    sessions[index].get("status_sequence", []),
                ),
            }
        )
    return output
...
def cluster_summary(
    ...
    return {
        ...
        "representative_events": sequence_events(sessions[representative_index]),
        "representative_status_summary": status_summary(
            sessions[representative_index].get("raw_sequence", []),
            sessions[representative_index].get("api_sequence", []),
            sessions[representative_index].get("status_sequence", []),
        ),
        "cluster_status_summary": merge_status_summaries(cluster_sessions_list),
        "length": {
